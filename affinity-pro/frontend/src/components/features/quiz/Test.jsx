...
const appDisplayName = (tab) =>
    tab === 'photo' ? 'Pixel (Photo) / ភីកសែល (រូបថត)'
    : tab === 'designer' ? 'Vector (Designer) / វ៉ិចទ័រ (ឌីហ្សាញ)'
    : 'Layout (Publisher) / ប្លង់ (ផាប់លីស៊ឺ)';
...
const APP_TABS = [
    { id: 'photo',     labelEn: 'Pixel (Photo)',     labelKh: 'ភីកសែល (រូបថត)',  Icon: Camera   },
    { id: 'designer',  labelEn: 'Vector (Designer)',  labelKh: 'វ៉ិចទ័រ (ឌីហ្សាញ)', Icon: PenTool  },
    { id: 'publisher', labelEn: 'Layout (Publisher)', labelKh: 'ប្លង់ (ផាប់លីស៊ឺ)', Icon: Book     },
];
...
const Test = ({ isDarkMode, isAdmin }) => {
    ...
    useEffect(() => {
        ...
        const n = localStorage.getItem('affinityPro_user_name');
        if (n) setUserName(n);
        const sc = safeParse('affinityPro_quiz_scores');   if (sc) setHighScores(sc);
        const ul = safeParse('affinityPro_quiz_unlocked'); if (ul) setUnlockedLevels(ul);
        const sl = safeParse('affinityPro_quiz_stars');    if (sl) setLevelStars(sl);
        const cd = safeParse('affinityPro_quiz_certs');    if (cd) setCertsData(cd);
        setDataLoaded(true);
    }, []);
    ...
    useEffect(() => {
        ...
        localStorage.setItem('affinityPro_quiz_unlocked', JSON.stringify(unlockedLevels));
        localStorage.setItem('affinityPro_quiz_stars',    JSON.stringify(levelStars));
        localStorage.setItem('affinityPro_quiz_scores',   JSON.stringify(highScores));
        localStorage.setItem('affinityPro_quiz_certs',    JSON.stringify(certsData));
        if (userName) localStorage.setItem('affinityPro_user_name', userName);
